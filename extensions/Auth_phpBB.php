    ...
    class Auth_phpBB extends AuthPlugin implements iAuthPlugin
    {
        ...
        private function connect()
        {
            ...
            if ($this->_UseExtDatabase == true)
            {
                ...
                $pConn = @mysqli_connect($this->_MySQL_Host, $this->_MySQL_Username, $this->_MySQL_Password, $this->_MySQL_Database);
            ...
            {
                // Connect to database.
                $pConn = mysqli_connect($GLOBALS['wgDBserver'], $GLOBALS['wgDBuser'], $GLOBALS['wgDBpassword'], true);
                ...
                $db_selected = mysqli_select_db($GLOBALS['wgDBname']);
            ...
            $this->_MySQL_Version = substr(mysqli_get_server_info($pConn), 0, 3); // Get the mysql version.
            mysqli_query($pConn, "SET NAMES 'utf8'"); // This is so utf8 usernames work. Needed for MySQL 4.1
    	...
    	public function getCanonicalName( $username )
    	{
            ...
            $username = ucfirst(str_replace(array('[',']',' '), array('','','_'), $username));
            ...
            while($faryMySQLResult = mysqli_fetch_assoc($pConn, $fresMySQLResult))
    	...
    	private function isMemberOfWikiGroup($username)
    	{
    		...
    		$pConn = $this->connect();
    	    $username = $this->utf8($username); // Convert to UTF8
    	    $username = mysqli_real_escape_string($pConn, $username); // Clean username.
    	    ...
    	    foreach ($this->_WikiGroupName as $WikiGrpName)
    	    {
                ...
                mysql_query('SELECT @isThere := COUNT( * ) FROM `' . $this->_User_GroupTB .
                            '` WHERE `user_id` = @userId AND `group_id` = @wikiId and `user_pending` = 0;', $pConn)
    	...
    	{
    	    throw new Exception($message . '<br />' . 'MySQL Error Number: ' . mysql_errno() . '<br />' . 'MySQL Error Message: ' . mysqli_error($pConn) . '<br /><br />');
    	}
...
$wgAuth_Config['WikiGroupName'] = 'Users';       // Name of your PHPBB group
                                                // users need to be a member
                                                // of to use the wiki. (i.e. wiki)
...
$wgAuth_Config['MySQL_Host']        = 'p:osudb';      // phpBB MySQL Host Name.
$wgAuth_Config['MySQL_Username']    = 'www';       // phpBB MySQL Username.
$wgAuth_Config['MySQL_Password']    = '';       // phpBB MySQL Password.
$wgAuth_Config['MySQL_Database']    = 'osu';       // phpBB MySQL Database Name.
...
$wgAuth_Config['PathToPHPBB']    = '/var/www/forum/';         // Path from this file to your phpBB install.

// Local
$wgAuth_Config['LoginMessage']   = '<b>You need an osu! account to login.</b><br /><a href="/forum/ucp.php?mode=register">Click here to create an account.</a>'; // Localize this message.
